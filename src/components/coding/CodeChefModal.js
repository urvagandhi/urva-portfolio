          ...
          >
            ...
            <div className="p-8 xs:p-4 pb-4 xs:pb-2 border-b border-solid border-dark/10 dark:border-light/10 flex flex-col md:gap-4 relative">
              ...
              <div className="flex items-center gap-4 sm:flex-col sm:items-start">
                <div className="relative w-20 h-20 xs:w-14 xs:h-14 rounded-2xl overflow-hidden border-2 border-solid border-red-500/50 shadow-md">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm text-dark/70 dark:text-light/70 flex-wrap">
                    <span className="font-semibold text-red-600 dark:text-red-400">
                      @{ccData?.info?.handle || "urva_gandhi"}
                    </span>
                    {ccData?.info?.country && (
                          ...
                          <div className="border border-solid border-dark/10 dark:border-light/10 p-6 rounded-2xl bg-light dark:bg-dark">
                            ...
                            <div className="space-y-3.5 text-sm font-semibold">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span>
                                  {ccData?.derivedMetrics?.longestStreak || "0"}{" "}
                                  days
                                </span>
                              </div>
                              <div className="flex justify-between">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span>
                                  {ccData?.derivedMetrics?.acceptanceRate ||
                                    "0"}
                                  %
                                </span>
                              </div>
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-6 p-4 mb-6 rounded-2xl bg-red-500/5 border border-solid border-red-500/10 shadow-sm">
                              ...
                              <div>
                                ...
                                <h6 className="text-base font-extrabold mt-0.5">
                                  {getCcRatingStyle(ccStars).name}
                                </h6>
                                <p className="text-xs text-dark/60 dark:text-light/60 mt-1 leading-snug">
                                  Dynamic rating-tier achievement badge based on
                                  active contest ratings.
                                </p>
                              </div>
                            </div>
                            ...
                            {ccData?.badges?.length > 0 ? (
                              <div className="space-y-4 max-h-[220px] overflow-y-auto pr-2">
                                {ccData.badges.map((badge, idx) => (
                                  <div
                                    key={idx}
                                    className="flex gap-4 p-3 rounded-xl hover:bg-dark/5 dark:hover:bg-light/5 transition-all duration-300 items-center"
                                  >
                                    ...
                                    <div className="flex-1">
                                      <h6 className="text-xs font-bold text-red-500 leading-tight">
                                        {badge.title}
                                      </h6>
                                      <p className="text-[10px] text-dark/60 dark:text-light/60 mt-1 leading-snug">
                                        {badge.description}
                                      </p>
                                    </div>
                                  </div>
                              ...
                              <div className="py-8 text-center text-sm font-semibold text-dark/40 dark:text-light/40">
                                No other badges earned yet
                              </div>
                            )}
                          </div>
                        </div>
                          ...
                          <div className="border border-solid border-dark/10 dark:border-light/10 rounded-2xl p-6 bg-light dark:bg-[#0d1117]/50">
                            ...
                            <div className="grid grid-cols-3 gap-6 md:grid-cols-1">
                              ...
                              <div className="col-span-2 relative h-[220px]">
                                {(() => {
                                  const sortedCcHistory = [
                                    ...ccData.contestHistory,
                                  ].sort(
                                    (a, b) =>
                                  ...
                                  const historyRatings = sortedCcHistory.map(
                                    (h) => h.rating,
                                  );
                                  const minR =
                                    Math.min(...historyRatings, 1000) - 30;
                                  const maxR =
                                    Math.max(...historyRatings, 1300) + 30;
                                  const rRange = maxR - minR;

                                  const points = sortedCcHistory.map((h, i) => {
                                    const x =
                                      padX +
                                      (i / (sortedCcHistory.length - 1 || 1)) *
                                        (svgWidth - 2 * padX);
                                    const y =
                                      svgHeight -
                                      padY -
                              ...
                              {(() => {
                                ...
                                const activeItem =
                                  hoveredCcContest ||
                                  sortedCcHistory[sortedCcHistory.length - 1];

                                const idx = sortedCcHistory.findIndex(
                                  (h) => h.contestId === activeItem.contestId,
                                );
                                const prevItem =
                                  idx > 0 ? sortedCcHistory[idx - 1] : null;
                                const delta = prevItem
                                  ? activeItem.rating - prevItem.rating
                                  : activeItem.rating - 1000;
                            ...
                            {ccData.recentSubmissions.map((sub, idx) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <span
                                    className={`px-3 py-1 rounded-full text-xs font-bold ${isAC ? "bg-green-500/10 text-green-500 border border-solid border-green-500/20" : "bg-red-500/10 text-red-500 border border-solid border-red-500/20"}`}
                                  >
