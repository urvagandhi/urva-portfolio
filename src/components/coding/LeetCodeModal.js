...
import { LeetCodeIcon, LinkArrow } from "../shared/Icons.js";
...
export const LeetCodeModal = ({ show, onClose, data }) => {
  ...
  // Easy/Med/Hard stats
  const easySolved =
    submitStats.find((q) => q.difficulty === "Easy")?.count || 0;
  const mediumSolved =
    submitStats.find((q) => q.difficulty === "Medium")?.count || 0;
  const hardSolved =
    submitStats.find((q) => q.difficulty === "Hard")?.count || 0;
  const totalSolved =
    submitStats.find((q) => q.difficulty === "All")?.count || 0;

  // Total counts on platform
  const easyTotal =
    data?.totalQuestions?.find((q) => q.difficulty === "Easy")?.count || 839;
  const mediumTotal =
    data?.totalQuestions?.find((q) => q.difficulty === "Medium")?.count || 1729;
  const hardTotal =
    data?.totalQuestions?.find((q) => q.difficulty === "Hard")?.count || 745;
  ...
  const totalSubmissions = data?.submitStats?.totalSubmissionNum?.find(
    (q) => q.difficulty === "All",
  )?.submissions;
  const acSubmissions = data?.submitStats?.acSubmissionNum?.find(
    (q) => q.difficulty === "All",
  )?.submissions;

  const acceptanceRate =
    totalSubmissions && acSubmissions
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {contest.rating && (
                      <span className="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-solid border-amber-500/20">
                        ...
                        {contest.badge?.name ? `${contest.badge.name} (` : ""}
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm text-dark/70 dark:text-light/70 flex-wrap">
                    <span className="font-semibold text-amber-600 dark:text-amber-400">
                      @{data?.profile?.username || "urva_gandhi"}
                    </span>
                    {profile.countryName && (
                                ...
                                {badges.map((badge, idx) => (
                                  ...
                                  >
                                    ...
                                    <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 hidden group-hover/badge:block w-48 bg-dark dark:bg-light text-light dark:text-dark text-[11px] py-2 px-3 rounded-xl shadow-xl text-center leading-normal z-30 border border-solid border-dark/10 dark:border-light/10 pointer-events-none">
                                      <div className="font-extrabold text-amber-500 mb-0.5">
                                        {badge.displayName}
                                      </div>
                                      {badge.hoverText &&
                                        badge.hoverText !==
                                          badge.displayName && (
                                          <div className="text-[10px] opacity-80 font-medium mb-1">
                                            {badge.hoverText}
                                          </div>
                                        )}
                                      {badge.creationDate && (
                                        <div className="text-[9px] opacity-60 font-bold">
                                          Earned: {badge.creationDate}
                                        </div>
                                      )}
                                    </div>
                                  </motion.div>
                            ...
                            <div className="space-y-3.5 text-sm font-semibold">
                              <div className="flex justify-between">
                                ...
                                <span className="text-dark dark:text-light">
                                  {derived.yearContributions || 0}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-dark/60 dark:text-light/60">
                                  Average Submissions Per Active Day
                                </span>
                                <span className="text-dark dark:text-light">
                                  {derived.avgSubmissionsPerActiveDay || 0}/day
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-dark/60 dark:text-light/60">
                                ...
                                <span className="text-dark dark:text-light text-amber-600 dark:text-amber-400 font-bold">
                                  {derived.mostActiveMonth || "N/A"}
                                </span>
                              </div>
                              {derived.mostActiveDay?.date && (
                                <div className="flex justify-between">
                                  ...
                                  <span className="text-dark dark:text-light">
                                    {derived.mostActiveDay.count} submissions (
                                    {derived.mostActiveDay.date})
                                  </span>
                                </div>
                              )}
                          ...
                          <div className="overflow-x-auto pb-2 no-scrollbar">
                            {(() => {
                              ...
                              for (
                                ...
                                const submissionsOnDay =
                                  leetcodeCalendarMap[dateStr] || 0;
                            ...
                            {languages.length > 0 ? (
                              languages.map((lang, idx) => {
                                const percentage =
                                  totalSolved > 0
                                    ? (
                                        (lang.problemsSolved / totalSolved) *
                                        100
                                      ).toFixed(1)
                                    : "0";
                                ...
                                if (lang.languageName === "Java")
                                  color = "bg-amber-600";
                                if (lang.languageName === "Python")
                                  color = "bg-green-500";
                                if (lang.languageName === "MySQL")
                                  color = "bg-purple-500";
                        ...
                        {contest.rating ? (
                            ...
                            <div className="border border-solid border-dark/10 dark:border-light/10 rounded-2xl p-8 bg-dark/5 dark:bg-light/5 flex items-center justify-between gap-8 md:flex-col md:items-stretch">
                              <div className="flex-1">
                                <h5 className="text-3xl font-extrabold flex items-center gap-2 text-amber-500">
                                  <Trophy className="w-8 h-8" /> Rating:{" "}
                                ...
                                <p className="mt-3 text-sm font-bold text-dark/70 dark:text-light/70">
                                  ...
                                  contests on LeetCode. Positioned in the top{" "}
                                  <span className="text-primary dark:text-primaryDark">
                                    {contest.topPercentage}%
                                  </span>{" "}
                                  of active participants globally!
                                </p>
                                ...
                                <div className="grid grid-cols-3 gap-4 mt-6 md:grid-cols-2 sm:grid-cols-1">
                                  <div className="border border-solid border-dark/10 dark:border-light/10 p-4 rounded-xl bg-light dark:bg-dark">
                                    <div className="text-xs font-bold text-dark/50 dark:text-light/50 uppercase">
                                      Highest Rating
                                    </div>
                                    <div className="text-2xl font-extrabold mt-1 text-amber-500">
                                      {leetcodeHighestRating}
                                    </div>
                                  </div>
                                  <div className="border border-solid border-dark/10 dark:border-light/10 p-4 rounded-xl bg-light dark:bg-dark">
                                    <div className="text-xs font-bold text-dark/50 dark:text-light/50 uppercase">
                              ...
                              <div className="w-48 h-48 md:w-full md:h-auto flex flex-col items-center justify-center border border-dashed border-dark/20 dark:border-light/20 rounded-2xl p-6 bg-light dark:bg-dark/40 shadow-sm relative overflow-hidden">
                                ...
                                <div className="text-xs font-bold text-dark/60 dark:text-light/60 mt-1">
                                  Top {contest.topPercentage}%
                                </div>
                                <div className="text-xs font-bold text-dark/60 dark:text-light/60 mt-1">
                                  Max: {leetcodeHighestRating}
                                </div>
                              </div>
                            </div>
                                ...
                                <div className="grid grid-cols-3 gap-6 md:grid-cols-1">
                                  ...
                                  <div className="col-span-2 relative h-[220px]">
                                    {(() => {
                                      ...
                                      const historyRatings = sortedHistory.map(
                                        (h) => h.rating,
                                      );
                                      const minR =
                                        Math.min(...historyRatings, 1500) - 30;
                                      const maxR =
                                        Math.max(...historyRatings, 1600) + 30;
                                      ...
                                      const points = sortedHistory.map(
                                        (h, i) => {
                                          const x =
                                            padX +
                                            (i /
                                              (sortedHistory.length - 1 || 1)) *
                                              (svgWidth - 2 * padX);
                                          const y =
                                            svgHeight -
                                            padY -
                                            ((h.rating - minR) / rRange) *
                                              (svgHeight - 2 * padY);
                                          return { x, y, ...h };
                                        },
                                      );
                                              ...
                                              <g key={idx}>
                                                <circle
                                                  ...
                                                  className={`fill-amber-500 transition-all duration-150 cursor-pointer ${
                                                    isSelected
                                                      ? "stroke-light dark:stroke-[#0d1117] stroke-2"
                                                      : "hover:scale-125"
                                                  }`}
                                                  onMouseEnter={() =>
                                                    setHoveredContest(p)
                                  ...
                                  {(() => {
                                    ...
                                    const prevRating =
                                      prevIdx >= 0
                                        ? sortedHistory[prevIdx].rating
                                        : 1500;
                                    const delta = Math.round(
                                      activeItem.rating - prevRating,
                                    );
                                        ...
                                        <div className="mt-4 space-y-2 text-xs font-semibold">
                                          <div className="flex justify-between">
                                            ...
                                            <span className="text-dark dark:text-light flex items-center gap-1">
                                              ...
                                              <span
                                                className={`text-[10px] font-extrabold ${delta >= 0 ? "text-emerald-500" : "text-red-500"}`}
                                              >
                                                {delta >= 0
                                                  ? `+${delta}`
                                                  : delta}
                                          ...
                                          <div className="flex justify-between">
                                            ...
                                            <span className="text-dark dark:text-light">
                                              {activeItem.problemsSolved} /{" "}
                                              {activeItem.totalProblems}
                                            </span>
                                          </div>
                                        </div>
                                      </div>
                          ...
                          <div className="border border-solid border-dark/10 dark:border-light/10 rounded-2xl p-8 flex flex-col items-center justify-center text-center gap-3">
                            ...
                            <p className="text-sm text-dark/60 dark:text-light/60 max-w-md">
                              This user hasn&apos;t participated in any official
                              LeetCode contests yet, or rating calculation is
                              pending.
                            </p>
                          </div>
                        )}
                      ...
                      >
                        {recentSubmissions.length > 0 ? (
                          <div className="border border-solid border-dark/10 dark:border-light/10 rounded-2xl overflow-hidden divide-y divide-solid divide-dark/10 dark:divide-light/10">
                            {recentSubmissions.map((sub, idx) => {
                              ...
                              return (
                                <div
                                  key={idx}
                                  className="p-4 flex items-center justify-between hover:bg-dark/5 dark:hover:bg-light/5 transition-colors duration-150"
                                >
                          ...
                          <div className="border border-solid border-dark/10 dark:border-light/10 rounded-2xl p-8 flex flex-col items-center justify-center text-center gap-3">
                            ...
                            <p className="text-sm text-dark/60 dark:text-light/60 max-w-md">
                              We couldn&apos;t load recent submissions for this
                              user right now.
                            </p>
                          </div>
                        )}
