...
export const GFGModal = ({ show, onClose, data }) => {
  ...
  const rank = `#${rankVal}`;
  const institute =
    userInfo.institute_name || "Nirma University (NU) Ahmedabad";
  ...
  const totalCalculated =
    schoolSolved + basicSolved + easySolved + mediumSolved + hardSolved;
  const totalDisplay = totalCalculated > 0 ? totalCalculated : totalSolved;

  const schoolPercent =
    totalDisplay > 0 ? Math.round((schoolSolved / totalDisplay) * 100) : 0;
  const basicPercent =
    totalDisplay > 0 ? Math.round((basicSolved / totalDisplay) * 100) : 0;
  const easyPercent =
    totalDisplay > 0 ? Math.round((easySolved / totalDisplay) * 100) : 0;
  const mediumPercent =
    totalDisplay > 0 ? Math.round((mediumSolved / totalDisplay) * 100) : 0;
  const hardPercent =
    totalDisplay > 0 ? Math.round((hardSolved / totalDisplay) * 100) : 0;
  ...
  const displayInstitute = gradYear
    ? `${institute} (Class of ${gradYear})`
    : institute;
                      ...
                      <>
                        ...
                        {articlesCount > 0 && (
                          <>
                            <span className="h-1 w-1 rounded-full bg-dark/20 dark:bg-light/20" />
                            <span>
                              <strong>{articlesCount}</strong> articles
                            </span>
                          </>
                        )}
                      </>
                ...
                {gfgLoading ? (
                  <div className="flex flex-col items-center justify-center py-20 gap-4">
                    ...
                    <span className="text-sm font-bold text-dark/50 dark:text-light/50">
                      Fetching GeeksforGeeks Analytics...
                    </span>
                  </div>
                ) : (
                  <>
                        ...
                        <div className="border border-solid border-dark/10 dark:border-light/10 p-6 rounded-2xl bg-light dark:bg-dark space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-4 pt-2 sm:grid-cols-1">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="text-xs text-dark/50 dark:text-light/50 font-bold uppercase">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="text-sm font-semibold">
                                  {userInfo.is_campus_ambassador
                                    ? "Yes, Campus Ambassador"
                                    : "Regular Member"}
                                </div>
                              </div>
                            </div>
                        ...
                        <div className="border border-solid border-dark/10 dark:border-light/10 p-6 rounded-2xl bg-light dark:bg-dark">
                          ...
                          <div className="overflow-x-auto pb-2 no-scrollbar">
                            {(() => {
                              ...
                              const generateGFGContributions = (
                                totalSolved,
                                currentStreak,
                                longestStreak,
                                username,
                                ...
                                for (
                                  let i = 0;
                                  i < Math.min(currentStreak, numDays);
                                  i++
                                ) {
                                  contributions[i] = 1;
                                }
                                ...
                                for (
                                  ...
                                  i <
                                  Math.min(
                                    longestStart + longestStreak,
                                    numDays,
                                  );
                                  i++
                                ) {
                                  contributions[i] = Math.max(
                                    contributions[i],
                                    1,
                                  );
                                }
                                ...
                                const currentSum = contributions.reduce(
                                  (a, b) => a + b,
                                  0,
                                );
                                let solvedLeft = Math.max(
                                  0,
                                  totalSolved - currentSum,
                                );

                                // 3. Generate stable deterministic weights (stable relative to today)
                                const dayWeights = [];
                                for (let i = 0; i < numDays; i++) {
                                  const dayRng = getSeedRandom(
                                    `${username || "urva_gandhi"}_day_${i}`,
                                  );
                              ...
                              const gfgContributions = generateGFGContributions(
                                totalSolved,
                                currentStreak,
                                longestStreak,
                                gfgData.username,
                          ...
                          <div className="mt-2 text-right text-[10px] text-dark/40 dark:text-light/40 font-bold">
                            * Activity map generated dynamically from total
                            solved counts
                          </div>
                        </div>
                      </motion.div>
                      ...
                      >
                        <div className="grid grid-cols-3 gap-4 sm:grid-cols-1">
                          <div className="border border-solid border-dark/10 dark:border-light/10 p-5 rounded-2xl bg-light dark:bg-dark text-center">
                            ...
                            <span className="block text-xs font-bold text-dark/50 dark:text-light/50 uppercase mt-1">
                              Current POD Streak
                            </span>
                          </div>
                          <div className="border border-solid border-dark/10 dark:border-light/10 p-5 rounded-2xl bg-light dark:bg-dark text-center">
                        ...
                        <div className="border border-solid border-dark/10 dark:border-light/10 p-6 rounded-2xl bg-light dark:bg-dark">
                          <h5 className="text-md font-bold mb-3 border-b border-solid border-dark/10 dark:border-light/10 pb-2">
                            What is Problem of the Day (POD)?
                          </h5>
                          <p className="text-sm leading-relaxed text-dark/70 dark:text-light/70">
                            GeeksforGeeks Problem of the Day (POD) is a daily
                            coding challenge aimed at improving consistency.
                            Every day, a selected problem from basic, easy,
                            medium, or hard difficulty is featured. Achieving
                            consistent streaks demonstrates persistent
                            dedication to coding discipline.
                          </p>
                          {globalLongestStreak > 0 && (
                            <div className="mt-4 p-3 rounded-xl bg-orange-500/5 border border-solid border-orange-500/10 text-xs font-semibold text-orange-600 dark:text-orange-400 flex items-center gap-2">
                              ...
                              <span>
                                Global platform longest streak reference value:{" "}
