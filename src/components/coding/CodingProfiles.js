...
const LeetCodeModal = dynamic(
  () => import("./LeetCodeModal").then((mod) => mod.LeetCodeModal),
  { ssr: false },
);
const CodeforcesModal = dynamic(
  () => import("./CodeforcesModal").then((mod) => mod.CodeforcesModal),
  { ssr: false },
);
const CodeChefModal = dynamic(
  () => import("./CodeChefModal").then((mod) => mod.CodeChefModal),
  { ssr: false },
);
const GFGModal = dynamic(
  () => import("./GFGModal").then((mod) => mod.GFGModal),
  { ssr: false },
);
const HackerRankModal = dynamic(
  () => import("./HackerRankModal").then((mod) => mod.HackerRankModal),
  { ssr: false },
);
...
export default function CodingProfiles() {
  ...
  const leetcodeHighestRating =
    leetcodeData?.contestHistory && leetcodeData.contestHistory.length > 0
      ? Math.round(
          Math.max(...leetcodeData.contestHistory.map((h) => h.rating)),
        ).toString()
      : "1616";
  ...
  const topLanguage =
    leetcodeData?.languages?.length > 0
      ? [...leetcodeData.languages].sort(
  ...
  const gfgScore = gfgData?.info?.score ? gfgData.info.score.toString() : "198";
  ...
  return (
    <div className="w-full mt-16">
      <h3 className="text-2xl font-bold dark:text-light mb-6">
