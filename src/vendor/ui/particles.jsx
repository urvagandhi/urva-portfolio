  ...
  const animate = () => {
    ...
    for (let i = circles.current.length - 1; i >= 0; i--) {
      ...
      if (distance < attractionRadius && distance > 0) {
        const pullStrength =
          Math.pow(1 - distance / attractionRadius, 2) * 80 * circle.magnetism;
        pullX = (distX / distance) * pullStrength;
        pullY = (distY / distance) * pullStrength;
      }

      const targetTranslateX =
        mouse.current.x / (staticity / circle.magnetism) + pullX;
      const targetTranslateY =
        mouse.current.y / (staticity / circle.magnetism) + pullY;
