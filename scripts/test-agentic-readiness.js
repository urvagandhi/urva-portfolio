...
async function runAudit(baseUrl) {
  ...
  try {
    ...
    const res404Md = await makeRequest(`${baseUrl}/nonexistent-path-test-12345`, { Accept: 'text/markdown' });
    assertTest(
    ...
    assertTest(
      ...
      res404Md.body.includes('llms.txt') && res404Md.body.includes('sitemap.xml'),
      `(Body snippet: ${res404Md.body.slice(0, 100)})`
    );

    // 2. Content Negotiation & Vary header compliance (acceptmarkdown.com)
    const resAboutMd = await makeRequest(`${baseUrl}/about`, { Accept: 'text/markdown' });
    assertTest(
      'Markdown Content Negotiation served markdown for Accept: text/markdown',
      resAboutMd.statusCode === 200 && (resAboutMd.headers['content-type'] || '').includes('text/markdown'),
    ...
    const resAboutHtml = await makeRequest(`${baseUrl}/about`, { Accept: 'text/html' });
    ...
    assertTest(
      ...
      resMcp.headers['mcp-version'] === '1.0' || (resMcp.body && resMcp.body.includes('schema_version')),
    ...
    const resAgentInstructions = await makeRequest(`${baseUrl}/.well-known/agent-instructions`);
    assertTest(
      '/.well-known/agent-instructions exists',
      resAgentInstructions.statusCode === 200 && resAgentInstructions.body.includes('When to Use This'),
